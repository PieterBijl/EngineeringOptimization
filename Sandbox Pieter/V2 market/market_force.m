...
tic;
while sum(abs(x1(:,i)-xsol)) > 0.05 & i<1000
    ...
    market_cost(:,i) = x1(:,i)*budget.*((energy_cost+1/capital_loan_duration*(build_cost+build_subsidies)).*misc_cost+subsidies);
...
function f = objfun(x)
    ...
    f = sum([x(1); x(2); x(3); x(4)].*(w_euro*(energy_cost+1/capital_loan_duration*(build_cost+build_subsidies).*misc_cost+subsidies)+w_CO2*CO2_cost));
end
